...
class Landmark:
    """Representation of a landmark locatio"""
...
def meters_to_km(meters: int) -> int:
    """Helper function for outputting kilometer"""
    kilometers = meters // 1000
    return int(kilometers)


def seconds_to_minutes(seconds: int) -> int:
    """Helper function for outputting minute"""
    minutes = round(seconds / 60)
    return int(minutes)


def seconds_to_hours_minutes(seconds: int) -> str:
    """Helper function for outputting travel tim"""
...
async def plan_route(routing_api: RoutingApi, start: Landmark, destination: Landmark) -> None:
    """Plan a route from one landmark to anothe"""
...
async def routes(api_key: str) -> None:
    """Generate all unique combinations of 2 landmarks and plan a route between the"""
